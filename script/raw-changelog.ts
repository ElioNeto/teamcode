...
const bot = ["actions-user", "github-actions[bot]", "teamcode", "teamcode-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/teamcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/teamcode/")) areas.add("core")
