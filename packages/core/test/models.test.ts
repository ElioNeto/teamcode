...
// test/preload.ts pins TEAMCODE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.TEAMCODE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.TEAMCODE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.TEAMCODE_MODELS_PATH = undefined
  Flag.TEAMCODE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.TEAMCODE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.TEAMCODE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
describe("ModelsDev Service", () => {
  ...
  it.live("refresh(false) fetches when on-disk file is stale", () =>
    Effect.gen(function* () {
      // Stale: mtime 61 minutes ago, beyond the 60-minute TTL.
      yield* writeCache(fixture, Date.now() - 61 * 60 * 1000)
      const state = yield* Ref.make({ ...initialState, body: JSON.stringify(fixture2) })
