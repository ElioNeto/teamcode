...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "teamcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "opencode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "teamcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "opencode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["teamcode.json", "opencode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "teamcode.json"),
        path.join(tmp.path, "opencode.jsonc"),
        path.join(project, "teamcode.json"),
