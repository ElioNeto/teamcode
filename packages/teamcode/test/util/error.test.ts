...
describe("util.error", () => {
  ...
  test("schema-backed named errors are real NamedError instances", () => {
    ...
    expect({ name: error._tag, data: { providerID: error.providerID, message: error.message } }).toEqual({ name: "ProviderAuthError", data: { providerID: "anthropic", message: "boom" } })
  })

  test("named errors without fields serialize data", () => {
    const err = new MessageError.OutputLengthError({})
    expect({ name: err._tag, data: {} }).toEqual({ name: "MessageOutputLengthError", data: {} })
  })
})
