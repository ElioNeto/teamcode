...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips teamcode-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-teamcode-directory": "/home/user/project",
          "x-teamcode-workspace": "ws_123",
          "accept-encoding": "gzip",
          "x-custom": "keep",
        },
      })
      const result = ProxyUtil.headers(req)
      expect(result.get("x-teamcode-directory")).toBeNull()
      expect(result.get("x-teamcode-workspace")).toBeNull()
