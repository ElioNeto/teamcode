...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.TEAMCODE_EXPERIMENTAL_DISABLE_FILEWATCHER
    ...
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.TEAMCODE_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.TEAMCODE_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
