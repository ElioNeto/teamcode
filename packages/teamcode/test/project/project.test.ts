...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const cacheFile = path.join(tmp, ".git", "teamcode")
  ...
  it.live("should handle git repository with commits", () =>
    Effect.gen(function* () {
      ...
      const cacheFile = path.join(tmp, ".git", "teamcode")
      expect(yield* Effect.promise(() => Bun.file(cacheFile).exists())).toBe(true)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "teamcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "teamcode")
      const wrongCache = path.join(parentDir, ".git", "teamcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "teamcode")
      const cacheB = path.join(bareB, "teamcode")
      const wrongCache = path.join(parentDir, ".git", "teamcode")
