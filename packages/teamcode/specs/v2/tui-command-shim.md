...
Delete `packages/teamcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/teamcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/teamcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/teamcode`
- TUI plugin loader tests in `packages/teamcode` if runtime plugin API wiring changed
