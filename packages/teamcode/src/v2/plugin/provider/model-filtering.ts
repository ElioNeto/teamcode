...
export const setupModelFiltering = Effect.gen(function* () {
  ...
  extraPluginRegistrations.push(
    Effect.gen(function* () {
      ...
      yield* registerFilteringHooks(pluginSvc, runtimeFlags, configInfo.provider ?? {})
    }),
  )
})
...
export const ModelFilteringPlugin = PluginV2.define({
  ...
  effect: Effect.succeed({
    "model.update": Effect.fn("ModelFiltering.model.update")(function* (evt) {
      // 1. Alpha / experimental filtering
      if (evt.model.status === "alpha") {
        const flags = yield* RuntimeFlags.Service
        if (!flags.enableExperimentalModels) {
          evt.cancel = true
          return
        }
      }
