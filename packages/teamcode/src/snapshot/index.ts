        ...
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          const ignore = Effect.fnUntraced(function* (files: string[]) {
            ...
            if (check.code !== 0 && check.code !== 1) {
              log.error("git check-ignore failed", { code: check.code, stderr: check.stderr.slice(0, 500) })
          ...
          const restore = Effect.fnUntraced(function* (snapshot: string) {
            return yield* locked(
              Effect.gen(function* () {
                ...
                if (result.code !== 0) {
                  log.error("failed to read-tree snapshot", { stderr: result.stderr })
                  return
                }
                ...
                if (checkout.code !== 0) {
                  log.error("failed to checkout-index snapshot", { stderr: checkout.stderr })
                  return
                }
              }),
            )
