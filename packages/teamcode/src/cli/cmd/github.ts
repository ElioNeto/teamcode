...
const WORKFLOW_FILE = ".github/workflows/teamcode.yml"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        async function getAppInfo() {
          ...
          // Get repo info — find the primary remote, not just "origin"
          const remotes = await Effect.runPromise(
            gitSvc.run(["remote"], { cwd: ctx.worktree }),
          ).then((x) => x.text().trim().split("\n").filter(Boolean))
          if (remotes.length === 0) {
            prompts.log.error(`No git remotes found. Please add a remote.`)
            throw new UI.CancelledError()
          }
          const remoteName =
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/teamcode-agent"
          const command =
...
jobs:
  opencode:
    if: |
      ...
      startsWith(github.event.comment.body, '/teamcode')
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/teamcode,/oc")
          .split(",")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("teamcode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `teamcode/${type}-${hex}-${timestamp}`
        }
        return `teamcode/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/teamcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
