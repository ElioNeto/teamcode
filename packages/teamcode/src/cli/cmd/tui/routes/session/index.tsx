...
const GO_UPSELL_PROVIDERS = new Set(["teamcode", "teamcode-go"])
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}teamcode -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
...
function Task(props: ToolProps<typeof TaskTool>) {
  const { navigate } = useRoute()
  const sync = useSync()
  const [sessionExists, setSessionExists] = createSignal(true)

  onMount(() => {
    if (props.metadata.sessionId && !sync.data.message[props.metadata.sessionId]?.length)
      sync.session.sync(props.metadata.sessionId).catch(() => setSessionExists(false))
  })
  ...
  return (
    <InlineTool
      ...
      part={props.part}
      onClick={() => {
        if (props.metadata.sessionId && sessionExists()) {
