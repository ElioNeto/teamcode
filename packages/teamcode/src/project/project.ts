...
import { NonNegativeInt, optionalOmitUndefined } from "@teamcode-ai/core/schema"
...
export class Service extends Context.Service<Service, Interface>()("@teamcode/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(flags.fakeVcs)
    ...
    const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(pathSvc.join(dir, "teamcode")).pipe(
        Effect.map((x) => x.trim()),
