...
import { InstallationLocal, InstallationVersion } from "@teamcode-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@teamcode-ai/core/schema"
...
export const Info = Schema.Struct({
  ...
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://teamcode.ai/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://teamcode.ai/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@teamcode-ai/core/schema`. See the definition
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://teamcode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      if (!flags.config && !flags.configDir && !flags.configContent) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://teamcode.ai/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://teamcode.ai/config.json"
