...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for teamcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal teamcode sessions
...
teamcode acp --cwd /path/to/project
```
...
```bash
TEAMCODE_ENABLE_QUESTION_TOOL=1 teamcode acp
```
...
- **Terminal Integration**: Full terminal support via teamcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | teamcode acp
```
...
ACP sessions map cleanly to teamcode's internal session model:
