import type { PermissionRequest, Session } from "@teamcode-ai/sdk/v2/client"
...
export function trimSessions(
  ...
  const nonArchivedRoots = roots.filter((s) => !s.time?.archived)
  const archivedRoots = roots.filter((s) => !!s.time?.archived)
  const base = nonArchivedRoots.slice(0, limit)
  const recent = takeRecentSessions(nonArchivedRoots.slice(limit), SESSION_RECENT_LIMIT, cutoff)
  const keepRoots = [...base, ...recent, ...archivedRoots]
