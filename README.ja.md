...
# パッケージマネージャー
npm i -g teamcode-ai        # bun/pnpm/yarn でもOK
scoop install teamcode             # Windows
choco install teamcode             # Windows
brew install teamcode/tap/teamcode # macOS と Linux（推奨。常に最新）
brew install teamcode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S teamcode            # Arch Linux (Stable)
paru -S teamcode-bin               # Arch Linux (Latest from AUR)
mise use -g teamcode               # どのOSでも
nix run nixpkgs#teamcode           # または github:teamcode/teamcode で最新 dev ブランチ
```
...
TeamCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/teamcode/teamcode/releases) から直接ダウンロードするか、[teamcode.ai/download](https://teamcode.ai/download) を利用してください。
...
1. `$TEAMCODE_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.teamcode/bin` - デフォルトのフォールバック
...
TeamCode には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://teamcode.ai/docs/agents) の詳細はこちら。

### ドキュメント

TeamCode の設定については [**ドキュメント**](https://teamcode.ai/docs) を参照してください。

### コントリビュート

TeamCode に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### TeamCode の上に構築する

TeamCode に関連するプロジェクトで、名前に "teamcode"（例: "teamcode-dashboard" や "teamcode-mobile"）を含める場合は、そのプロジェクトが TeamCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/teamcode) | [X.com](https://x.com/teamcode)
